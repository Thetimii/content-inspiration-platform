...
export async function GET(request: Request) {
  try {
    ...
    const envVars = {
      ...
      OPENROUTER_API_KEY: process.env.OPENROUTER_API_KEY ? `set (length: ${process.env.OPENROUTER_API_KEY.length}, starts with: ${process.env.OPENROUTER_API_KEY.substring(0, 5)}...)` : 'not set',
