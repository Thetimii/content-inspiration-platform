 ...
 * This version extracts a frame from the video and passes it to the AI
 */
export async function POST(request: Request) {
  try {
    ...
    analyzeVideoInBackground(videoId, video.cover_url || video.video_url);
    ...
    console.error('Unexpected error in direct-analyze-frame route:', error);
 ...
 */
async function analyzeVideoInBackground(videoId: string, imageUrl: string) {
  try {
    ...
    const prompt = `Analyze this TikTok video thumbnail in detail. Please provide:
1. A comprehensive analysis of what's likely happening in the video based on this frame
2. Key visual elements and objects present
3. Any text or captions visible in the image
