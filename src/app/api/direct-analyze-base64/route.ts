 ...
 * This version downloads the video and sends it as base64 in the request
 */
export async function POST(request: Request) {
  try {
    ...
    console.error('Unexpected error in direct-analyze-base64 route:', error);
...
async function analyzeVideoInBackground(videoId: string, videoUrl: string) {
  try {
    ...
    console.log('Converting video buffer to base64');
    const base64Video = Buffer.from(videoBuffer).toString('base64');
    console.log(`Base64 video length: ${base64Video.length} characters`);
...
Be specific and detailed in your analysis.`;
    ...
    try {
      ...
      // Prepare the request payload with base64 encoded video
      ...
      console.log('Request payload structure (not showing full base64):', {
        ...
        messages: [
          {
            ...
            content: [
              ...
              { 
                type: "image_url", 
                image_url: { 
                  url: `data:video/mp4;base64,[BASE64_DATA_TRUNCATED]` 
                } 
              }
            ]
          }
      ...
      const openRouterResponse = await axios.post(
        ...
        requestPayload,
        {
          ...
          maxContentLength: Infinity // Allow large response body
        }
      );
